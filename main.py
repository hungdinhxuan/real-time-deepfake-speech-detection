...
def run(rank, world_size, port, yml_cfg, args):
    ...
    train_loader = getDataLoader(dataset=train_set, batch_size=exp_config.batch_size_train, num_workers=sys_config.num_workers)
    ...
    if ckpt is not None:
        ...
        model.load_state_dict(f_state_dict_wrapper(state_dict, data_parallel=True))
...
if __name__ == "__main__":
    ...
    if is_eval:
        ...
        if args.score_all_folder_path is not None:
            ...
            list_all_checkpoint = [ckpt for ckpt in list_all_checkpoint if '.pt'  in ckpt]
        ...
        if is_score:
            ...
            if sys_config.model in globals():
                ...
                raise ValueError(f"Model {sys_config.model} not found.")
            model = torch.nn.DataParallel(model)
            model.load_state_dict(f_state_dict_wrapper(torch.load(ckpt, map_location=device), data_parallel=True ))
            model = model.module
